...
def parser():
    ...
    p.add_argument("--include-northern-zones", action="store_true")
    p.add_argument("--force-refresh", action="store_true", help="Use quota for a fresh result; only while parked")
    p.add_argument("--confirm-force-refresh", action="store_true", help="Second confirmation when cached traffic is still fresh")
    p.add_argument("--driving", action="store_true", help="Suppress unsafe force-refresh controls")
...
def main():
    ...
    if args.mode in ("pre_shift", "live_refresh"):
        ...
        result = service.mobility.run(args.mode, args.current_zone, args.latitude, args.longitude, args.include_distant_zones, args.include_northern_zones, args.force_refresh, args.confirm_force_refresh, args.driving)
